...
>>> X = np.random.random_integers(0, num_classes+1, size=(num_batch, input_seq_len)).astype('int32')
>>> Y = np.random.random_integers(0, num_classes, size=(num_batch, output_seq_len)).astype('int32')
>>> output = train(X,Y)
...
def cost(y_hat_softmax, y, y_mask=None, mask=None):
    """
